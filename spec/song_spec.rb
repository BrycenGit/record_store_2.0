...
before(:each) do
  ...
  @album.save()
  @artist = Artist.new({:name => "Alma", :id => nil})
  @artist.save()
end

  describe('#==') do
    it("is the same song if it has the same attributes as another song") do
      song = Song.new({:name => "Naima", :album_id => @album.id, :id => nil, :artist_id => @artist.id})
      song2 = Song.new({:name => "Naima", :album_id => @album.id, :id => nil, :artist_id => @artist.id})
  ...
  describe('.all') do
    it("returns a list of all songs") do
      song = Song.new({:name => "Giant Steps", :album_id => @album.id, :id => nil, :artist_id => @artist.id})
      song.save()
      song2 = Song.new({:name => "Naima", :album_id => @album.id, :id => nil, :artist_id => @artist.id})
      song2.save()
  ...
  describe('.find_by_album') do
    it("finds songs for an album") do
      ...
      song = Song.new({:name => "Naima", :album_id => @album.id, :id => nil, :artist_id => @artist.id})
      song.save()
      song2 = Song.new({:name => "California", :album_id => album2.id , :id => nil, :artist_id => @artist.id})
      song2.save()
